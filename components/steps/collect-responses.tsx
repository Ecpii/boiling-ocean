...
import { WorkflowStep, FAILURE_MODES, type ConversationTurn, type ModelResponse } from "@/lib/types"
...
export function CollectResponses() {
  ...
  const enabledQuestions = state.questions.filter((q) => q.enabled && q.text.trim())
  ...
  async function collectAllResponses() {
    ...
    for (let i = 0; i < enabledQuestions.length; i++) {
      ...
      const modeLabel = FAILURE_MODES.find((fm) => fm.id === q.failureMode)?.label ?? q.failureMode
      setCurrentQuestion(q.text)
      setCurrentMode(modeLabel)
      setProgress(((i) / totalQuestions) * 100)
  ...
  const progressPercent = totalQuestions > 0 ? ((completedCount + failedCount) / totalQuestions) * 100 : 0
      ...
      <Card className="w-full max-w-2xl">
        <CardContent className="flex flex-col gap-6 py-8">
          <div className="text-center">
            <h2 className="text-2xl font-semibold tracking-tight">Collecting Model Responses</h2>
            <p className="text-muted-foreground mt-1">
              Testing your model with {totalQuestions} questions across {FAILURE_MODES.length} failure modes
            </p>
          </div>
            ...
            <div className="rounded-lg border bg-background p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <Badge variant="outline" className="mb-2">{currentMode}</Badge>
                  <p className="text-sm text-muted-foreground line-clamp-2">{currentQuestion}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {!state.isLoading && progressPercent === 100 && (
            <div className="text-center">
